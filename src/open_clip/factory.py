...
def load_state_dict(
    ...
    if str(checkpoint_path).endswith(".safetensors"):
        ...
        try:
            ...
            # Handle both TypeError (old PyTorch) and RuntimeError (weights_only issues)
            if weights_only and "weights_only" in str(e):
                logging.warning(f"Failed to load with weights_only=True: {e}")
                logging.warning("Retrying with weights_only=False. Only do this with trusted checkpoints.")
                try:
                    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
                except Exception as fallback_e:
                    raise RuntimeError(f"Failed to load checkpoint even with weights_only=False: {fallback_e}") from e
            else:
...
def create_model(
        ...
        enable_ibm_module: bool = False, # Use IBM module for CLIP (if available)
    ...
    if enable_ibm_module:
        model_cfg['use_ibm'] = True
...
def create_loss(args):
    if args.enable_ibm_module:
        from open_clip.loss import ClipWithIBMLoss
        return ClipWithIBMLoss(
...
def create_model_and_transforms(
    ...
    model = create_model(
        model_name,
        pretrained,
        enable_ibm_module=enable_ibm_module,
