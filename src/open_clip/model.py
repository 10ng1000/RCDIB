...
class InformationBottleneckModule(nn.Module):
    def __init__(self, input_dim, latent_dim, output_dim):
        ...
        if input_dim == 768:
            hidden_dims = [512, 384]
        if input_dim == 1024:
            hidden_dims = [768, 512, 384]
        # 多层版的参数量
        # RN50模型 embed_dim=1024
        # if input_dim == 1024:
        #     hidden_dims = [1024, 896, 768, 640, 512, 384, 256]
        # if input_dim == 512:
        #     hidden_dims = [512, 448, 384, 320, 256]
        # if input_dim == 768:
        #     hidden_dims = [768, 640, 512, 384, 256]
        modules = []
        in_channels = input_dim
        if enable_chebyKAN:
            for h_dim in hidden_dims:
                modules.append(nn.Sequential(
                    ChebyKANLinear(in_channels, h_dim, degree=3),
        ...
        if enable_chebyKAN:
            self.hidden_mean = ChebyKANLinear(hidden_dims[-1], latent_dim)
            self.hidden_logvar = ChebyKANLinear(hidden_dims[-1], latent_dim)
        else:
            self.hidden_mean = nn.Linear(hidden_dims[-1], latent_dim)
            self.hidden_logvar = nn.Linear(hidden_dims[-1], latent_dim)
    ...
    def forward(self, x):
        ...
        # encoded = x
        # 添加噪声
        mu, log_var = self.hidden_mean(encoded), self.hidden_logvar(encoded)
        std = torch.exp(0.5 * log_var)
        eps = torch.randn_like(std)
        # eps = torch.cuda.FloatTensor(std.size()).normal_()
        noisy_encoded = mu + eps * std
        # 解码
        reconstructed = self.decoder(noisy_encoded)
        reconstructed = F.normalize(reconstructed, dim=-1)
        return reconstructed, noisy_encoded, mu, log_var
...
class CLIP(nn.Module):
    ...
    def __init__(
            ...
            # 新增参数
            use_ibm: bool = False,
            ibm_latent_dim: int = 256,
    ):
        super().__init__()
        self.vision_cfg = copy.deepcopy(vision_cfg)
        self.text_cfg = copy.deepcopy(text_cfg)
        ...
        # 新增Information Bottleneck模块
        self.use_ibm = use_ibm
        
        if use_ibm:
            self.image_ibm = InformationBottleneckModule(
                input_dim=embed_dim, 
                latent_dim=ibm_latent_dim, 
                output_dim=embed_dim
            )
        
            self.text_ibm = InformationBottleneckModule(
                input_dim=embed_dim, 
                latent_dim=ibm_latent_dim, 
                output_dim=embed_dim
            )
    ...
    def encode_image(self, image, normalize: bool = False):
        ...
        normalized_features = F.normalize(features, dim=-1) if normalize else features
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        normalized_features = F.normalize(x, dim=-1) if normalize else x
    ...
    def forward(
        ...
        image_features = image_mu = image_log_var = None
        text_features = text_mu = text_log_var = None
    
        if image is not None:
            image_features = self.encode_image(image, normalize=True)
            if  self.use_ibm:
                reconstructed_image_features, image_z, image_mu, image_log_var = self.image_ibm(image_features)
            
                
        if text is not None:
            text_features = self.encode_text(text, normalize=True)
            if self.use_ibm:
                reconstructed_text_features, text_z, text_mu, text_log_var = self.text_ibm(text_features)
        ...
        if self.output_dict:
            ...
            if self.use_ibm:
                out_dict['reconstructed_image_features'] = reconstructed_image_features
                out_dict['image_mu'] = image_mu
                out_dict['image_log_var'] = image_log_var
                out_dict['reconstructed_text_features'] = reconstructed_text_features
                out_dict['text_mu'] = text_mu
                out_dict['text_log_var'] = text_log_var
                out_dict['image_z'] = image_z
                out_dict['text_z'] = text_z
            if self.logit_bias is not None:
                out_dict['logit_bias'] = self.logit_bias
            return out_dict
        
        if self.use_ibm:
            return image_features, text_features, reconstructed_image_features, reconstructed_text_features, self.logit_scale.exp(), image_mu, image_log_var, text_mu, text_log_var, image_z, text_z
